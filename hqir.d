...
struct FunctionValue {
	...
	FunctionValue getSpent(CCGen ccg, QCGen qcg) {
		auto r = this;
		r.ccg = ccg;
		r.qcg = qcg;
		r.qCapMovedT = [];
...
private bool typeHasClassical(Expression ty) {
	...
	return ast_ty.hasClassicalComponent(ty);
}
...
struct RetValue {
	...
	this(Value cret) {
		this.classicalRet = cret;
	}
	this(Value cret, Value qret) {
		this.classicalRet = cret;
		this.quantumRet = qret;
	}
	...
	RetValue toQuantum(CondRet retCond, CondRet retCondQ, ScopeWriter w) {
		...
		if(classicalRet.hasQuantum) {
			...
			if(!quantumRet) {
				...
				qreg = w.qcg.withCond(CondAny(retCond.condC)).addCond(CondAny(retCondQ.condQ), qreg); // [retCond.condC,retCondQ.condQ]
				auto qqreg = quantumRet.qreg; // [!retCond.condC,retCond.condQ]
				auto wq = w.withCond(w.nscope, CondAny(retCond.condQ));
				qqreg = wq.qcg.withCond(CondAny(retCond.condC.invert()))
					.addCond(CondAny(retCondQ.condQ), qqreg); // [!retCond.condC,retCond.condQ,retCondQ.condQ]
				qqreg = w.qcg.withCond(CondAny(retCond.condC.invert())).withCond(CondAny(retCondQ.condQ))
					.removeCond(CondAny(retCond.condQ), qqreg); // [!retCond.condC,retCondQ.condQ]
				qreg = w.withCond(w.nscope, CondAny(retCondQ.condQ))
					.qcg.cmerge(retCond.condC, qqreg, qreg); // [retCondQ.condQ]
			}
		}
		return RetValue(null, Value.newReg(creg, qreg));
	}
		...
		if(cond.isQuantum) {
			if(c0 && c1) {
				...
				if(q0) {
					...
					q0 = wc0.qcg.withCond(CondAny(c0, false)).allocQubit(0);
				}
				auto o0 = wc0.qcg.withCond(CondAny(c0)).allocQubit(1);
				q0 = wc0.qcg.cmerge(CondC(c0), q0, o0);

				auto wc1 = w1.withCond(w1.nscope, CondAny(condC, false));
				if(q1) {
					q1 = w1.qcg.withCond(CondAny(c1, false)).addCond(CondAny(condC, false), q1);
				} else {
					q1 = wc1.qcg.withCond(CondAny(c1, false)).allocQubit(0);
				}
				auto o1 = wc1.qcg.withCond(CondAny(c1)).allocQubit(1);
...
class CCGen {
	...
	CReg qfuncInner2(CReg r) {
		assert(!!r);
		return boxIndex(ctypeSilqQFunc, 3, r, 0);
	}
...
class ScopeWriter {
	...
	Value valAddCond(CondAny cond, Value v) {
		auto creg = v.creg;
		auto qreg = v.qreg ? qcg.addCond(cond, v.qreg) : null;
		return Value.newReg(creg, qreg);
	}

	Value valRemoveCond(CondAny cond, Value v) {
		auto creg = v.creg;
		auto qreg = v.qreg ? qcg.removeCond(cond, v.qreg) : null;
		return Value.newReg(creg, qreg);
	}
	...
	Value implCall(CallInfo ci) {
		...
		if(ci.newFunctionVar) {
			...
			auto funcC = ccg.qfuncInner2(targetC);
			...
			if(ci.targetF) {
				...
				fval.func = ci.target.func.getSpent(ccg, qcg);
	...
	Result genStmts(Expression[] stmts, Result result = Result.passes()) {
		...
		Result combineResults(Result ra, Result rb, ScopeWriter scB) {
			...
			if(!ra.retValue.quantumRet && !rb.retValue.classicalRet) {
				assert(!ra.retCond.condQ && !rb.retCond.condC);
				return Result.conditionallyReturns(
					RetValue(ra.retValue.classicalRet, rb.retValue.quantumRet),
					CondRet(ra.retCond.condC, rb.retCond.condQ)
				);
			}
			auto cA = ra.retCond, cB = rb.retCond;
			auto rvA = ra.retValue, rvB = rb.retValue;
			auto vB = cB.asCondRetValue(scB);
			if(!rvA.quantumRet) {
				assert(!cA.condQ);
				auto ccA = cA.condC, rcA = rvA.classicalRet; // [ccA]
				assert(rcA && ccA);
				auto ccB = cB.condC, rcB = rvB.classicalRet; // [!ccA,ccB]
				auto cqB = cB.condQ, rqB = rvB.quantumRet;   // [!ccA,!ccB,cqB]
				assert(rcB && ccB);
				CReg condC = ccg.cond(ccA, vB.condC, ctx.boolTrue);
				...
				auto condQ = cqB;
				if(condQ) {
					condQ = CondQ(qcg.withCond(CondAny(ccA.invert())).withCond(CondAny(ccB.invert())).dup(condQ.reg), condQ.value);
					condQ.reg = qcg.withCond(CondAny(ccA.invert())).withCond(CondAny(ccB.invert())).addCond(CondAny(condC, false), condQ.reg); // [!ccA,!ccB,!condC]
					condQ.reg = qcg.withCond(CondAny(ccA.invert())).withCond(CondAny(condC, false)).removeCond(CondAny(cB.condC.invert()), condQ.reg); // [!ccA,!condC]
					condQ.reg = qcg.withCond(CondAny(condC, false)).removeCond(CondAny(ccA.invert()), condQ.reg); // [!condC]
				}
				...
				cB.forget(scB);
				cA.forget(this);
				return Result.conditionallyReturns(RetValue(cret, qret), CondRet(CondC(condC), condQ));
			} else {
				auto vBq = vB.dup(scB).toQuantum(scB);
				rvB = rvB.toQuantum(rb.retCond, vBq.asCondRet(), scB);
				assert(!rvB.classicalRet && rvB.quantumRet);
				vB.asCondRet().forget(scB);
				auto condC = cA.condC, cret = rvA.classicalRet; // [condC]
				auto cqA = cA.condQ, rqA = rvA.quantumRet; // [!condC,cqA]
				auto cqB = vBq.condQ, rqB = rvB.quantumRet; // [!condC,!cqA,cqB]

				CondQ condQ = CondQ.init;
				Value qret = null;
				if(cqA && cqB) {
					assert(rqA && rqB && cA.condQ);
					auto wq = condC ? withCond(nscope, CondAny(condC.invert())) : this;
					auto cqF = wq.qcg.withCond(CondAny(cqA.invert())).dup(cqB);
					auto cqT = wq.qcg.withCond(CondAny(cqA)).allocQubit(1);
					condQ = CondQ(wq.qcg.merge(CondAny(cqA), cqF, cqT));
					rqA = wq.withCond(wq.nscope, CondAny(cA.condQ)).valAddCond(CondAny(condQ), rqA); // [!condC, cqA, condQ]
					rqB = wq.withCond(wq.nscope, CondAny(cA.condQ.invert()))
						.withCond(wq.nscope, CondAny(cqB))
						.valAddCond(CondAny(condQ), rqB); // [!condC, !cqA, cqB, condQ]
					rqB = wq.withCond(wq.nscope, CondAny(cA.condQ.invert()))
	...
	CReg genMeasure(Expression ty, Value arg, bool mayUseRTTI = true) {
		...
		if(auto prodTy = cast(ast_ty.ProductTy) ty) {
			...
			if(hasConst ^ hasMoved) {
				...
				auto cfunc = ccg.qfuncInner2(arg.creg);
				if(!arg.hasQuantum) return cfunc;
	...
	void delegate() packIndirect(CallInfo ci, ref CReg[] cCall, ref QReg[] qcCall, ref QReg[] qiCall, bool isReversed) {
		...
		if(ci.hasQuantum) {
			...
			if(hasConst ^ hasMoved) {
				...
				auto qfuncC = ccg.qfuncInner2(targetC);
	...
	Value implConvert(ast_conv.FunctionConversion conv, Value v) {
		...
		assert(!!targetQType ^ (targetQTypeConst && targetQTypeMoved));
		return valNewQ(targetQType ? ccg.qfuncPack(newC, targetQType) : ccg.qfuncPack2(newC, targetQTypeConst, targetQTypeMoved), targetQ);
	}

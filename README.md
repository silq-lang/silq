...
1. Run 'dependencies.sh' to download the DMD D compiler from http://downloads.dlang.org/releases/2014/dmd.2.071.0.zip and unzip it.
...
- variable = expression; assigns to an existing variable. +=, -=, *= and /= are also supported.
...
- The following operators are supported and denote the corresponding operations on real numbers with standard C operator precedence:
...
dparse.d:
  ...
  (Used for debugging and the sampleFrom primitive, aims to invert the default toString method for distribution expressions,
  while also allowing different notations).
